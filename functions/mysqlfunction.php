...
function getCategoriesLevelWise($currentCatId) {
    
    $selParCat = selectqry('*', TB_CATEGORIES, array('parent_id='=>'0'), 'title');
    
    while ($resParCat = mysqli_fetch_assoc($selParCat)) :
        
            $isActive = "";
            if($resParCat["id"] == $currentCatId){ $isActive = " active"; }
            $categoryListForFilter .= '<a class="dropdown-item'.$isActive.'" href="'.URL_BASE.SEO_JOBS."/jcategory/".$resParCat["id"].'">'.$resParCat["title"].'</a>';
            
                //Second level category
                $selCat = selectqry('*', TB_CATEGORIES, array('parent_id=' => $resParCat['id']), 'title');
                while ($resCat = mysqli_fetch_array($selCat)) :
                        $isActive = "";
                        if($resCat["id"] == $currentCatId){ $isActive = " active"; }
                        $categoryListForFilter .= '<a class="dropdown-item'.$isActive.'" href="'.URL_BASE.SEO_JOBS."/jcategory/".$resCat["id"].'">&nbsp;&nbsp;&nbsp;&nbsp;'.$resCat["title"].'</a>';
                endwhile;
...
//function getCategoriesLevelWise($parentId, $currentCatId) {
//    
//    $retVal = array($parentId);    
//    $selParCat = selectqry('*', TB_CATEGORIES, array('parent_id='=>$parentId), 'title');
//    
//    $space = "";
//    
//    if($count > 0) {
...
//            if($resParCat["id"] == $currentCatId){ $isActive = " active"; }
//
//            $categoryListForFilter .= '<a class="dropdown-item'.$isActive.'" href="'.URL_BASE.SEO_JOBS."/jcategory/".$resParCat["id"].'">'.$space.$resParCat["title"].'</a>';
//            
//            if( $resParCat["parent_id"] > 0 ) {
//                
//                    $parent =  fetchqry('id', TB_CATEGORIES, array("id="=>$resParCat["parent_id"]) );
//                    $parentId = $parent['parent_id'];
//                    echo $parentId;
//                    
//                    if($parentId > 0){
//                            $count += 1;
//                            getCategoriesLevelWise($parentId, $resParCat['id']);
//                    }                    
//            }
//
//    }
...
//                    
//                    if($parentId > 0){
//                        return getCategoriesLevelWise($parentId, $resParCat['id']);
//                    }                    
//            }
//
//    }
